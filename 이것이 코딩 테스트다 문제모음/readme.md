...
4. 계수 정렬 : '데이터의 크기 범위가 제한되어 정수 형태로 표현할 수 있을 때'만 사용 가능, 데이터가 1,000,000개 이하일 때(메모리 초과)  
...
## 서로소 집합 개념
- 공통 원소가 없는 두 집합 : (1,2) 와 (3,4) | (1,2) 와 (2,3) 은 서로소 집합이 아님.
...
## 크루스칼 알고리즘(무방향) - find_parent, union_parent function 기억
...
3. 새롭게 진입차수를 정렬하여 0인 노드를 q에 모두 삽입.
4. q가 빌때까지 반복

**모든 원소를 방문하기 전 큐가 빈다면 사이크링 존재**  
**위상정렬의 답은 여러개가 될 수 있음을 인지**
