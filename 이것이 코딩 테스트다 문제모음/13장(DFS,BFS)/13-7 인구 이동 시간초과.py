...
def bfs(graph, queue, visited, number_of_area):
    #print(visited)
    now_graph_visited = [[False for _ in range(N)] for _ in range(N)]
    sum_of_sub_graph = 0
    sub_graph = []
    ...
    while queue:
        ...
        if visited[row][column] or now_graph_visited[row][column]: # 전체 그래프에서 방문한 적이 있는 노드라면 패스 or 현재 그래프에서 방문한 적 있으면 패스
            continue
        
        else: # 첫 방문 노드라면
            visited[row][column] = True # 전체 그래프에서 현재 노드 방문 처리
            now_graph_visited[row][column] = True # 현재 그래프에서 현재 노드 방문 처리
    ...
    #sub_graph = [[row,column] for row in range(N) for column in range(N) if now_graph_visited[row][column] == True]
    #sum_of_sub_graph = sum([graph[row][column] for row, column in sub_graph])
    after_population = int(sum_of_sub_graph / number_of_area)
    
    #new_graph = [row.copy() for row in graph]
    
    for row,column in sub_graph:
        #new_graph[row][column] = after_population
        graph[row][column] = after_population
    
    #return new_graph, visited, number_of_area
    return graph, visited, number_of_area
...
while(True):
    ...
    past_graph = [row.copy() for row in new_graph]
    for start_point in start_points: # 모든 점에 대하여 국경선 open 확인(=graph형성), 인구수 갱신
        ...
        if not visited[row][column]: # 처음 방문하는 노드인 경우 sub graph 생성
            ...
            #past_graph = [row.copy() for row in new_graph]
            number_of_area = 0
            new_graph, visited, number_of_area = bfs(past_graph, queue, visited, number_of_area) # new_graph는 인구수 갱신된 그래프.
            
    # graph와 new_graph 변함이 없다면 인구 이동 x
    #print('origianl : ', original_graph)
    #print('before loop : ', past_graph)
    #print('after loop : ', new_graph)
    check = sum([1 for row in range(N) for column in range(N) if original_graph[row][column] != new_graph[row][column]])
    if check == 0: #두 그래프가 같다면
        break
    else:
        #print('update graph!')
        count+=1
